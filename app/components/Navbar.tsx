...
export default function Navbar({ activePage }: { activePage: string }) {
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[#5a1f25]" style={{ background: "#6D2932" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-xl bg-[#E8D8C4] flex items-center justify-center text-[#6D2932] font-bold text-xl shadow-lg shadow-black/20">
              S
            </div>
            <div>
            ...
            <Link
              ...
              className={`px-3 py-1.5 rounded-full transition-colors ${
                activePage === item.href
                  ? "text-[#6D2932] bg-[#E8D8C4] font-semibold"
        ...
        <div className="flex items-center gap-3">
          <button className="relative p-2 text-[#E8D8C4] hover:bg-white/10 rounded-full transition-colors" aria-label="Notifikasi">
            <IconBell size={20} />
            <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-[#6D2932]"></span>
          </button>
        </div>
      </div>
    </header>
